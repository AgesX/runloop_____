...
class Solution_Time_Limit_Exceeded_orderOfLargestPlusSign{
    ...
    func orderOfLargestPlusSign(_ n: Int, _ mines: [[Int]]) -> Int {
        ...
        if even{
            ...
            result = max(check([half + 1, half + 1], in: mines, from: edgeMax, within: n), result)
            if result >= edgeMax{
                return result
            }
            while j < half{
...
extension Solution_Time_Limit_Exceeded_orderOfLargestPlusSign{
    func test(){
        let result = Solution_Time_Limit_Exceeded_orderOfLargestPlusSign().orderOfLargestPlusSign(3, [[0,0]])
